...
function pass(su) {
   ...
   try {
      ...
      paragraph.style.display = "block"; uvrayis.style.display = "block"; image.style.display = "block";

      showcurrentlocData.className = "weather-card active-card";

      // Injecting weather details into the paragraph element
      paragraph.innerHTML = ` < div class = "weather-info" > <h3 > $ {
         datais.location.name
      },
      $ {
         datais.location.region
      } < /h3> 
            ...
            <div class="stat-box green">Wind Speed: <b>${datais.current.wind_kph} km/h < /b></div > <div class = "stat-box blue" > UV Index: <b > $ {
         datais.current.uv
      } < /b></div > </div>`;

        / / Setting image source and processing UV index via your helper
      function image.src = datais.current.condition.icon; const {
         uvIndex,
         condition,
         color
      } = uvhelper(datais.current.uv); uvrayparagraph.style.backgroundColor = color; uvrayparagraph.innerHTML = ` < b > $ {
         uvIndex
      } < /b>, ${condition}`;

        / / Appending elements to the container icon.prepend(image); showcurrentlocData.append(paragraph); uvrayis.append(uvrayparagraph); getlocation.style.display = "none";
...
function fail(fu) {
   ...
   button.addEventListener('click', async() = >{
      try {
         ...
         const datais = await worker.json(); const warning = document.getElementById("warning");

         // Error handling for invalid location names
         if (datais.error ? .code === 1006) {
            ...
            warning.className = "warning-text api-error";
               ...
               $ {
                ...
                uvrayparagraph.innerHTML = `<b>${uvIndex}</b > ,
               $ {
                  condition
               }`;
